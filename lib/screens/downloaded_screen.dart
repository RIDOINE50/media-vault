...
class _DownloadedScreenState extends State<DownloadedScreen> with TickerProviderStateMixin {
  ...
  bool _isRefreshing = false; // ✅ NOUVEAU : pour éviter les appels multiples
  ...
  void _onDownloadChanged() {
    ...
    if (completed.isNotEmpty && !_isRefreshing) {
      print('🔄 Téléchargements terminés détectés: ${completed.length}');
  ...
  // ✅ CORRIGÉ : Plus de vérification _isLoading au début
      ...
      builder: (context, settings, child) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            ...
            actions: [
              IconButton(
                icon: Icon(Icons.refresh_rounded, color: isDark ? Colors.white : Colors.black87, size: 28),
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: Row(
          ...
          children: [
            Icon(icon, size: 20, color: isSelected ? Colors.white : (isDark ? Colors.grey[400] : Colors.grey[700])),
            const SizedBox(width: 8),
            Text(
              label,
              style: TextStyle(
  ...
  Widget _buildEmptyState(bool isDark) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.cloud_download_rounded, size: 100, color: isDark ? Colors.grey[700] : Colors.grey[400]),
          const SizedBox(height: 24),
          Text('Aucun téléchargement', style: TextStyle(color: isDark ? Colors.white : Colors.black87, fontSize: 20, fontWeight: FontWeight.w700)),
          const SizedBox(height: 12),
          Text('Va dans l\'onglet Rechercher pour télécharger', style: TextStyle(color: isDark ? Colors.grey[500] : Colors.grey[600], fontSize: 14)),
        ],
      ),
    );
  }
  ...
  Widget _buildShimmerLoading(bool isDark) {
    return Center(
      child: Column(
        mainAxisAlignment: MainAxisAlignment.center,
        children: [
          Icon(Icons.download_rounded, size: 80, color: Theme.of(context).primaryColor),
          const SizedBox(height: 24),
          Text('Chargement des téléchargements...', style: TextStyle(color: isDark ? Colors.white70 : Colors.black87, fontSize: 16)),
        ],
      ),
    );
  }
          ...
          child: Row(
            children: [
              Container(
                ...
                child: Icon(file.isVideo ? Icons.movie_rounded : Icons.music_note_rounded, color: file.isVideo ? Colors.blue : primaryColor, size: 36),
              ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(file.title, maxLines: 2, overflow: TextOverflow.ellipsis, style: TextStyle(color: isDark ? Colors.white : Colors.black87, fontWeight: FontWeight.w700, fontSize: 15)),
                    const SizedBox(height: 4),
                    Text(file.artist, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(color: isDark ? Colors.grey[400] : Colors.grey[600], fontSize: 13)),
                    ...
                    Row(
                      children: [
                        Container(
                          ...
                          child: Text(file.format.toUpperCase(), style: TextStyle(color: primaryColor, fontSize: 11, fontWeight: FontWeight.w800)),
                        ),
                        const SizedBox(width: 10),
                        Text(_formatBytes(fileSize), style: TextStyle(color: isDark ? Colors.grey[400] : Colors.grey[700], fontSize: 12)),
                      ],
                    ),
                  ],
                ),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          Row(
            children: [
              Icon(Icons.storage_rounded, color: primaryColor, size: 20),
              const SizedBox(width: 12),
              Text('Espace utilisé', style: TextStyle(color: isDark ? Colors.white70 : Colors.black87, fontSize: 14, fontWeight: FontWeight.w600)),
            ],
          ),
          Text(_getTotalSize(), style: TextStyle(color: primaryColor, fontWeight: FontWeight.w800, fontSize: 14)),
        ],
      ),
    );
  }
